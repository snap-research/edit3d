...
class NPYLoaderN(Dataset):
    def __init__(self, filelist, npoints_fine=2048, npoints_coarse=2048, only_sketch=False, imsize=64):
        self.filelist = filelist
        self.npoints_fine = npoints_fine
        self.npoints_coarse = npoints_coarse
        self.transform = transforms.Compose([transforms.Resize((imsize,imsize)), transforms.ToTensor(),])
        self.only_sketch = only_sketch
        print('[NPYLoaderN] Number of shapes: {}; #fine: {}; #coarse: {}.'.format(len(filelist), npoints_fine, 
                    npoints_coarse))   
    ...
    def __getitem__(self, idx):
        ...
        return {'surface_samples': data_f, 'sphere_samples' : data_c, 
                "sketch": data_im, "color_2d": data_color,
                'shape_indices': idx_t, 'shape_ids': shape_id}
...
def get_data_loaders(args):
                ...
                if (shape_id in sphere_list) and (shape_id in color2d_path.keys()) and (shape_id in im_path.keys()):
                    ...
                    if shape_id in train_split:
                        train_data_list.append((shape_id, surface_path, sphere_path, 
                                                im_path[shape_id], color2d_path[shape_id]))
    ...
    train_dataset = NPYLoaderN(train_data_list, args.train.num_sample_points.fine, 
    ...
    test_dataset = NPYLoaderN(test_data_list, args.test.num_sample_points.fine, 

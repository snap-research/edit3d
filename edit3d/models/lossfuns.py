...
# mse between generative images and the groundtruth images
def mse(pred, gt):
...
def binary_cross_entropy(pred, gt):
    return F.binary_cross_entropy_with_logits(pred, gt)


def bce_lpips(pred, gt):
    alpha = 1.0
    bce_loss = F.binary_cross_entropy_with_logits(pred, gt)
    percept_loss = lpips.LPIPS(net="vgg").to(edit3d.device)
    loss = bce_loss + percept_loss(pred, gt).mean() * alpha
    return loss

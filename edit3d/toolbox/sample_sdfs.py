...
def get_mesh_loader(split_file, data_dir):
    ...
    if split_file:
                ...
                if dataset.lower() == 'shapenetv2':
                    ...
                    for l1_name, l2 in l1.items():
                        for idx, l2_name in enumerate(l2):
                            file_path = os.path.join(data_dir, dataset, l1_name, l2_name, "models", "model_normalized.obj")
                            try:
                                next_id = l2[idx+1]
                            except IndexError:
                                next_id = None
        ...
        npy_list = list(filter(lambda x: x.is_file() and os.path.splitext(npy_path)[1] == '.npy', os.scandir(data_dir)))
        total = len(npy_list)
        for idx, npy_path in enumerate(npy_list):
            logger.info(f"Loading mesh: {npy_path}")
            current_id = os.path.splitext(os.path.basename(npy_path))[0]
            try:
                next_id = os.path.splitext(os.path.basename(npy_list[idx+1]))[0]
            except IndexError:
                next_id = None
            yield (lambda: numpy.load(npy_path.path)), current_id, next_id, completed/total
...
def main(args):
    ...
    for loader, shapeid, next_shapeid, progress in get_mesh_loader(args.split_file, args.mesh_npy_path):
        ...
        try:
            mesh = meshpreprocess_bsphere(loader()).to(device)
            if not args.notrim:

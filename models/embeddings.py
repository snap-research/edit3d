    ...
    def forward(self, idx, **kwargs):
        ...
        if self.training:
            ...
            if self.cfg.augment_latent:
                ...
                batch_latent_aug = mu.unsqueeze(1) + eps_aug*std.unsqueeze(1) # shape = (batch_sz, num_pts, latent_dim)
            elif self.cfg.sample_twice:
                eps_aug = torch.randn_like(std)
                batch_latent_aug = mu + eps_aug*std
            else:
                batch_latent_aug = batch_latent
            return {'latent_code': batch_latent, 'latent_code_augment': batch_latent_aug, 

...
class mineGAN(nn.Module):
    def __init__(self, vad, code_type, nz=128):
        ...
        self.vad = vad
        self.shape_miner = nn.Sequential(nn.Linear(nz, nz),
                                         nn.BatchNorm1d(nz),
                                         nn.ReLU(),
                                         nn.Linear(nz, nz),)
        self.color_miner = nn.Sequential(nn.Linear(nz, nz),
                                         nn.BatchNorm1d(nz),
                                         nn.ReLU(),
                                         nn.Linear(nz, nz),)
        if code_type == "shape":
...
TRAIN_STAGE = True if args.mode == 'train'
TEST_STAGE = True if args.mode == 'test'
...
if TRAIN_STAGE:
    ...
    if args.code == "shape":
        ...
        optimizerM = optim.Adam(list(netM.color_miner.parameters())+list(netM.shape_miner.parameters()), 
    ...
    for epoch in range(args.niter):
        for i, data in enumerate(dataloader, 0):
            ...
            print('Stage 1: [%d/%d][%d/%d] Loss_D: %.4f Loss_M: %.4f Loss_KLD: %.4f D(x): %.4f D(M(z)): %.4f / %.4f'
                  % (epoch, args.niter, i, len(dataloader),
                     errD.item(), errM.item(), loss_kld.item(), D_x, D_M_z1, D_M_z2))
        if (epoch+1 >= 50) and ((epoch+1) % 10 == 0):
            if not has_save:
                for real_i in range(real_cpu.size(0)):
                    vutils.save_image(real_cpu[real_i],'%s/real_%d.png' % (args.outf, real_i),normalize=True)
                    has_save = True
            fake, feat_shape, feat_color= netM(fixed_shape, fixed_color) 
            render_batch(netM.vad, feat_shape, feat_color, '%s/fake_all_epoch_%03d.png' % (args.outf, epoch))
            vutils.save_image(fake.detach(), '%s/fake_2d_epoch_%03d.png' % (args.outf, epoch), normalize=True)

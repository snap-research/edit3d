...
class Trainer(BaseTrainer):
    def __init__(self, cfg, args, device):
        ...
        # Init optimizers
        self.optim_deepsdf, self.lrscheduler_deepsdf = self._get_optim(self.deepsdf_net.parameters(),  # model params
                                                                       self.cfg.trainer.optim_deepsdf) # hyper-params
        ...
        # Init optimizers
        self.optim_colorsdf, self.lrscheduler_colorsdf = self._get_optim(self.colorsdf_net.parameters(),  # model params
                                                                       self.cfg.trainer.optim_colorsdf) # hyper-params
        # sketch generator
        ...
        # Init optimizers
        self.optim_imgen, self.lrscheduler_imgen = self._get_optim(self.imgen_net.parameters(),  # model params
                                                                   self.cfg.trainer.optim_imgen) # hyper-params
        ...
        # Init optimizers
        self.optim_colorgen, self.lrscheduler_colorgen = self._get_optim(self.colorgen_net.parameters(),  # model params
                                                                   self.cfg.trainer.optim_imgen) # hyper-params
    ...
    def prep_train(self): # czz: assign each instance a unique idx
        ...
        self.optim_latentcode_shape, self.lrscheduler_latentcode_shape = self._get_optim(self.latent_embeddings_shape.parameters(), self.cfg.trainer.optim_latentcode)
        
        # color embedding
        self.latent_embeddings_color = self._get_latent(self.cfg.trainer.latent_code_color, N=len(self.sid2idx))
        self.optim_latentcode_color, self.lrscheduler_latentcode_color = self._get_optim(self.latent_embeddings_color.parameters(), self.cfg.trainer.optim_latentcode)
    ...
    def _get_optim(self, parameters, cfg):
        if cfg.type.lower() == "adam":
            optim = torch.optim.Adam(parameters, lr=cfg.lr, betas=cfg.betas, eps=cfg.eps, 
    ...
    def _b_idx2latent(self, latent_embeddings, indices, num_augment_pts=None):
        ...
        return batch_latent, batch_latent_aug, kld # by default, batch_latent is simply the mu
    ...
    def step(self, data):
        ...
        latent_codes_coarse_shape, latent_codes_fine_shape, kld_shape = self._b_idx2latent(
                        self.latent_embeddings_shape, data_indices, num_augment_pts=data_f.size(1)) # [64 128]

        latent_codes_coarse_color, latent_codes_fine_color, kld_color = self._b_idx2latent(
                        self.latent_embeddings_color, data_indices, num_augment_pts=data_f.size(1)) # [64 128]
        ...
        loss = 0.5 * (loss_fine_shape * self.cfg.trainer.loss_fine_shape.weight + 
        ...
        (loss + kld_shape*self.cfg.trainer.kld_weight_shape + kld_color*self.cfg.trainer.kld_weight_color).backward()
        ...
        log_info = {'loss': loss.item(), 'loss_sketch': loss_sketch, 
                    'loss_shape':loss_fine_shape, 'loss_color3D': loss_color3D,
    ...
    def sample_images(self, data):
        ...
        latent_codes_coarse_shape, latent_codes_fine_shape, kld_shape = self._b_idx2latent(self.latent_embeddings_shape,
                                            data_indices, num_augment_pts=2048) # [64 128]
        ...
        latent_codes_coarse_color, latent_codes_fine_color, kld = self._b_idx2latent(self.latent_embeddings_color,
                                            data_indices, num_augment_pts=2048) # [64 128]
        ...
        return {"gt_sketch":data_sketch, "gen_sketch": sketch_samples,
                "gt_color": data_color, "gen_color": rgb_samples,

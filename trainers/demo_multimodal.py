...
class Trainer(CrossModalTrainer):
    ...
    def get_known_latent(self, idx):
        ...
        # shape
        latent_codes_coarse_shape, latent_codes_fine_shape, kld = self._b_idx2latent(self.latent_embeddings_shape, data_indices, num_augment_pts=1) # [64 128]
        loss_kld_shape = torch.mean(0.5 * torch.mean(latent_codes_coarse_shape**2, dim=-1))
        self.stats_loss_kld_shape = loss_kld_shape.item()
        # color
        latent_codes_coarse_color, latent_codes_fine_color, kld = self._b_idx2latent(self.latent_embeddings_color, data_indices, num_augment_pts=1) # [64 128]
    ...
    def manip_fun(self, x, target, mask, feat, gamma=0.02, beta=0.5, alphas=[1.0, 1.0, 0.0]):
        ...
        loss_man = alphas[0] * torch.mean(torch.abs(x - target) * mask.to(x.device)) +\
                   alphas[1] * loss_kld 
    ...
    def sample_latent_gaussian(self, num_pts):
        ...
        return latent_codes_coarse_shape["latent_code"], latent_codes_coarse_color["latent_code"]

    def step_manip_sketch(self, feat_shape, target, mask=None, gamma=0.02, beta=0.5, alphas=[1.0, 1.0, 0.0], epoch=1001):
        if mask == None:
        ...
        for i in range(epoch):
            ...
            loss_manip, loss_kld= self.manip_fun(sketch, target, mask, latent_codes_coarse, 
                                gamma=gamma, beta=beta, alphas=alphas)
            loss_manip.backward()
            if i % 100 == 0:
    ...
    def step_recon_rgb(self, feat_shape, feat_color, target, mask=None, epoch=1001, gamma=0.02, beta=0.5):
        if mask == None:
            mask = torch.ones_like(target)
        mask = mask.to(self.device)
        latent_codes_color = feat_color.to(self.device).clone().detach().requires_grad_(True)
        latent_codes_shape = feat_shape.to(self.device).clone().detach().requires_grad_(True)
        optim, lrscheduler = self._get_optim([latent_codes_shape, latent_codes_color], 
                                              self.cfg.manip.optim_rgb)
        latent_codes = []
        for param in self.colorgen_net.parameters():
                param.requires_grad = False
        target = target.to(self.device)
        for i in range(epoch):
            optim.zero_grad()
            color_2d = self.forward_color2d_grad(latent_codes_color, latent_codes_shape)
            loss_recon, loss_kld, loss_kld2 = self.recon_fun(color_2d, target, mask, 
                                        latent_codes_shape, latent_codes_color,
    ...
    def step_edit_rgb(self, feat_shape, feat_color, target, mask=None, epoch=1001, gamma=0.02, beta=0.5):
        if mask == None:
            mask = torch.ones_like(target)
        mask = mask.to(self.device)
        latent_codes_shape = feat_shape.to(self.device).clone().detach().requires_grad_(False)

...
def get_mask(source, target):
    mask = 1.*((target - source) != 0) # 0 means the unmodified region
    return mask
...
def main(args, cfg):
    ...
    for imname in os.listdir(source_dir):
        ...
        for editid in range(1, 10):
            print(editid)
            data = load_image_and_sketch(source_path, editid, prefix) 
            ...
            # editing
            edit_latent, color_code = edit(trainer, source_latent, data["source"], data["target"],
                                args.epoch, args.trial, args.gamma, args.beta)
            for iteration, latent_snap in enumerate(edit_latent[:10]):
                save(trainer, (latent_snap, color_code), data["source"], data["target"],
                     targetdir, imname+f"_{iteration}", save_ply=False)
